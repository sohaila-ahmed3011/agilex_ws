...
void GlobalMapperRos::InitSubscribers()
{
  ...
  velo_sub_ = pnh_.subscribe("/velodyne_points", 1, &GlobalMapperRos::VelodyneCallback, this);
...
{
  ROS_INFO("Mapper:: Velodyne received");

  if (!got_velodyne_points_)
  {
    got_velodyne_points_ = true;
  }
  ...
  try
  {
    transform_stamped = tf_buffer_.lookupTransform(target_frame, velodyne_msg->header.frame_id,
                                                   ros::Time(velodyne_msg->header.stamp), ros::Duration(0.12));
  }
  catch (tf2::TransformException& ex)
  {
    ROS_WARN("[GlobalMapperRos::VelodyneCallback] %s", ex.what());
    return;
  }
...
void GlobalMapperRos::Run()
{
  ...
  while (ros::ok())
  {
    ...
    else if (!got_velodyne_points_)
    {
      process_status.SetStatus(fla_msgs::ProcessStatus::ALARM);
      process_status.SetArg(ProcessArgs::NO_VELO_POINTS);
    } 
